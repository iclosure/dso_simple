...
*       _DrawHLine
*/
static void _DrawHLine(int y, int x0, int x1) {
  if (x0 < x1) {
    LCD_HL_DrawHLine(y, x0, x1);
  } else {
    LCD_HL_DrawHLine(y, x1, x0);
  }
}
...
static void _Flush(int x0, int y) {
  ...
  for (i=0; i<GL_FP_Cnt; i++) {
    int xNew = _aX[i];
    if (On) {
      LCD_HL_DrawHLine(y, x0 + x, x0 + xNew);
    }
    On ^= 1;
    x = xNew;
  }
...
static void _AddPoint(int x, int y, int y0, int y1, int xOff, int yOff) {
  if ((y0 ^ y1) >= 0) {
    x += xOff;
    LCD_HL_DrawHLine(y + yOff, x, x);    /* V-point, not crossing the polygon */
  } else {
    _Add(x);
  }
}
...
*/
void GUI_FillPolygon(const GUI_POINT*paPoint, int NumPoints, int xOff, int yOff) {
  int i, y;
  int yMin = GUI_YMAX;
  int yMax = GUI_YMIN;
    ...
    for (i=0; i<NumPoints; i++) {
      ...
      if (y0 == y) {
        if (y1 == y) {  /* Add the entire line */
          _DrawHLine(y + yOff, (paPoint + i )->x + xOff , (paPoint + i1)->x + xOff);
