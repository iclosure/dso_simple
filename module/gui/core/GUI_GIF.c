      ...
      if ((Index == OldIndex) && (XPos <= XEnd)) {
        ...
        if (Cnt) {
          if (OldIndex != Transparency) {
            ...
            LCD_DrawHLine(YPos, XPos - Cnt - 1, XPos - 1);
          } else if (Disposal == 2) {
          ...
          if (OldIndex >= 0) {
            if (OldIndex != Transparency) {
			  SETPIXEL(XPos - 1, YPos, *(pTrans + OldIndex));
            } else if (Disposal == 2) {
			  SETPIXEL(XPos - 1, YPos, BkColorIndex);
            }
          }
        }
      }
...
static void _ClearUnusedPixels(int x0, int y0, IMAGE_DESCRIPTOR * pDescriptor, GUI_GIF_IMAGE_INFO * pInfo) {
  LCD_SetColorIndex(LCD_BKCOLORINDEX);
