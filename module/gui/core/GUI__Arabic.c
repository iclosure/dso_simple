...
static int _GetTableIndex(U16 c) {
  if (c < 0x621)  return 0;
  if (c > 0x6cc)  return 0;
  if ((c >= 0x621) && (c <= 0x63a)) return c - 0x621;
  if ((c >= 0x641) && (c <= 0x64a)) return c - 0x641 + 26;
  if (c == 0x67e) return 36;
  if (c == 0x686) return 37;
  if (c == 0x698) return 38;
  if (c == 0x6a9) return 39;
  if (c == 0x6af) return 40;
  if (c == 0x6cc) return 41;
                  return 0;
}
...
static int _GetLigature(U16 Char, U16 Next, int PrevAffectsJoining) {
	...
	if (PrevAffectsJoining) {
		switch (Next) {
		case 0x622: return 0xfef6;
		case 0x623: return 0xfef8;
		case 0x625: return 0xfefa;
		case 0x627: return 0xfefc;
		}
	} else {
		switch (Next) {
...
static int _GetStringDistXArabic(const char GUI_UNI_PTR *s, int MaxNumChars) {
  U16 Glyph, Char, PrevChar = 0, NextChar=0;
  int CharIsArabic=0, IgnoreNextCharacter = 0, xSizeArabic = 0;
  while (--MaxNumChars >= 0) {
    if (!PrevChar) {
...
int GUI__GetLineDistXArabic(const char GUI_UNI_PTR *s, int MaxNumChars) {
  ...
  if (s) {
    U16 Glyph, Char, PrevChar = 0, NextChar=0;
    int CharIsArabic=0, IgnoreNextCharacter = 0;
...
void GUI__DispLineArabic(const char GUI_UNI_PTR * s, int MaxNumChars, const GUI_RECT * pRect) {
  ...
  #endif
    U16 Glyph, Char, PrevChar = 0, NextChar=0;
    int CharIsArabic=0, xPosArabicStart=0, IgnoreNextCharacter = 0, xSizeArabic;
    const char GUI_UNI_PTR * p;
    const char GUI_UNI_PTR * pNext=NULL;
    while (--MaxNumChars >= 0) {
      if (!PrevChar) {
        p        = s;
        Char     = GUI_UC__GetCharCodeInc(&s);
...
int GUI__GetCursorPosXArabic(const char GUI_UNI_PTR * s, int Index, int MaxNumChars) {
  U16 Glyph, Char, PrevChar = 0, NextChar=0, Cnt = 0;
  int CharIsArabic=0, ArabicMode = 0, IgnoreNextCharacter = 0; 
  int xSizeArabic=0, CursorPosXArabic = 0, LigaturPosition=0;
  const char GUI_UNI_PTR * p;
  const char GUI_UNI_PTR * pNext=NULL;
  while (--MaxNumChars >= 0) {
    if (!PrevChar) {
...
int GUI__GetCursorPosCharArabic(const char GUI_UNI_PTR * s, int x, int MaxNumChars) {
  U16 Glyph, Char, PrevChar = 0, NextChar=0, Cnt = 0, NumChars;
  int CharIsArabic=0, ArabicMode = 0, IgnoreNextCharacter = 0;
  int xSizeArabic=0, CursorPosXArabic = 0, CharWidth, NewPosX;
  const char GUI_UNI_PTR * p;
  const char GUI_UNI_PTR * pNext=NULL;
