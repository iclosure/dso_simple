...
static void _DrawBitline(int x0, int y0, int xsize, int BPP, const U8*pData, int Diff, const LCD_PIXELINDEX* pTrans) {
  ...
  while (xsize >0) {
    ...
    LCDDEV_DrawBitmap(x0 + Diff * LCD_XMAG, y0, NumPixels * LCD_XMAG, LCD_YMAG, BPP, 0, Buffer.ab, 0, pTrans);
                           ...
                           const LCD_PIXELINDEX* pTrans)
{
  ...
  for (iLine = 0; iLine < ysize; iLine++) {
    #if LCD_XMAG == 1
      LCDDEV_DrawBitmap(x0, y0, xsize, LCD_YMAG, BPP, 0, pData, Diff, pTrans);
    #else
...
*       LCD_MAG_DrawHLine
*/
void LCD_MAG_DrawHLine(int x0, int y0,  int x1) {
  x0 *= LCD_XMAG;
  x1 = x1*(LCD_XMAG) + LCD_XMAG-1;
  y0 *= LCD_YMAG;
  LCD_FillRect(x0, y0, x1, y0 + LCD_YMAG-1);
}
...
*       LCD_MAG_DrawVLine
*/
void LCD_MAG_DrawVLine(int x0 , int y0,  int y1) {
  y0 *= LCD_YMAG;
  y1 = y1*(LCD_YMAG) + LCD_YMAG-1;
  x0 *= LCD_XMAG;
  LCD_FillRect(x0, y0, x0 + LCD_XMAG-1, y1);
}
...
/*********************************************************************
*
*       LCD_MAG_GetPixelIndex
*/
unsigned int LCD_MAG_GetPixelIndex(int x, int y) {
  return LCD_GetPixelIndex(x* LCD_XMAG, y* LCD_YMAG);
}

/*********************************************************************
*
*       LCD_MAG_SetPixelIndex
*/
void LCD_MAG_SetPixelIndex(int x, int y, int ColorIndex) {
  ...
  for (iy=0; iy< LCD_YMAG; iy++) {
    for (ix=0; ix< LCD_XMAG; ix++) {
      LCD_SetPixelIndex(x+ix, y+iy, ColorIndex);
    }
  }
}
...
*       LCD_MAG_XorPixel
*/
void LCD_MAG_XorPixel(int x, int y) {
  int ix, iy;
  y *= LCD_YMAG;
  x *= LCD_XMAG;
  for (iy=0; iy< LCD_YMAG; iy++) {
    for (ix=0; ix< LCD_XMAG; ix++) {
      LCD_XorPixel(x+ix, y+iy);
    }
  }
}
