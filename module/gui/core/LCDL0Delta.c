...
static void _DrawBitLine1BPP(int x, U8 const GUI_UNI_PTR * p, int Diff, int xsize, const LCD_PIXELINDEX * pTrans) {
  ...
  case LCD_DRAWMODE_TRANS:
    do {
        ...
        _SetPixelIndex(xx, LCD_GetPixelIndex(xx, _y));
      }
      xx++;
			if (++Diff == 8) {
        Diff = 0;
  ...
  case LCD_DRAWMODE_XOR:;
    do {
  		if (*p & (0x80 >> Diff)) {
        int Pixel = LCD_GetPixelIndex(xx, _y);
  ...
  LCDDEV_DrawBitmap(x, _y, _Bm.XSize, 1, _Bm.BitsPerPixel, _Bm.BytesPerLine, _Bm.pData, 0, 0);
}
...
*/
void LCD_DELTA_DrawBitmap(int x0, int y0,
                             int xsize, int ysize,
                             int BitsPerPixel, 
                             int BytesPerLine,
...
void LCD_DELTA_FillRect(int x0, int y0, int x1, int y1);

/*********************************************************************
*
*       LCD_L0_DELTA_DrawHLine
*/
void LCD_DELTA_DrawHLine(int x0, int y,  int x1) {
  if (GUI_Context.DrawMode & LCD_DRAWMODE_XOR) {
    for (; x0 <= x1; x0++) {
      LCD_XorPixel(x0, y);
    }
  } else {
    LCD_DELTA_FillRect(x0, y, x1, y);
  }
}
...
*/
void LCD_DELTA_DrawVLine(int x , int y0,  int y1) {
  if (GUI_Context.DrawMode & LCD_DRAWMODE_XOR) {
    for (; y0 <= y1; y0++) {
      LCD_XorPixel(x, y0);
    }
    ...
    aIndex[1] = (x + 1) % 3;
    for (; y0 <= y1; y0++) {
      LCD_SetPixelIndex(x, y0, _aRGB[aIndex[y0 & 1]]);
    }
  }
}
  ...
  if (GUI_Context.DrawMode & LCD_DRAWMODE_XOR) {
    ...
    for (y = y0; y <= y1; y++) {
      ...
      if (Index) {
        ...
        LCDDEV_DrawBitmap(x0, y, _Bm.XSize, 1, _Bm.BitsPerPixel, _Bm.BytesPerLine, &_aPixelData_1[x0], 0, 0);
      } else {
        if (!aInit_0) {
          for (x = x0; x <= x1; x++) {
        ...
        LCDDEV_DrawBitmap(x0, y, _Bm.XSize, 1, _Bm.BitsPerPixel, _Bm.BytesPerLine, _Bm.pData, 0, 0);
     }
    }
  }
}
...
void LCD_DELTA_SetPixelIndex(int x, int y, int ColorIndex) {
  LCD_SetPixelIndex(x, y, ColorIndex);
}
