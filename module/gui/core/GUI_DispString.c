...
void GUI_DispStringHLeftAt(const char GUI_UNI_PTR *s, int x, int y) {
	int Align;
	Align = GUI_SetTextAlign((GUI_Context.TextAlign & ~GUI_TA_LEFT &~GUI_TA_CENTER) | GUI_TA_RIGHT);
	GUI_DispStringAt(s, x, y);
	GUI_SetTextAlign(Align);
}
/*********************************************************************
...
void GUI_DispStringHRightAt(const char GUI_UNI_PTR *s, int x, int y) {
	int Align;
	Align = GUI_SetTextAlign((GUI_Context.TextAlign & ~GUI_TA_RIGHT &~GUI_TA_CENTER) | GUI_TA_LEFT);
	GUI_DispStringAt(s, x, y);
	GUI_SetTextAlign(Align);
}
...
void GUI_ClearStringAt(const char GUI_UNI_PTR *s, int x, int y) {
	...
	r1.x0=x; r1.y0=y;
	r1.x1=x+GUI_GetStringDistX(s);
	r1.y1=y+GUI_GetFontSizeY();
	GUI_SetClipRect(&r1);
	GUI_ClearRectEx(r1);
	GUI_SetClipRect(&r0);
}
